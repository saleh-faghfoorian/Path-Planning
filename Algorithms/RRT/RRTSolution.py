    ...
    def solve(self):
        ...
        for k in range(len(self.points) - 1):
            ...
            for i in range(nodeNumbers):
                if abs(nodes[len(nodes) - 1].coord.x - goalNode.coord.x) < 5 and abs(nodes[len(nodes) - 1].coord.y - goalNode.coord.y) < 5:
                    break
            ...
            [self.trajectoryX.append(xGenerated[j]) for j in range(len(xGenerated))]
            [self.trajectoryY.append(yGenerated[j]) for j in range(len(yGenerated))]
...
def generateRandomNode(nodes, obstacles, boundary):
    ...
    while True:
        ...
        internalPoints = generateInternalPoints(nearNode, newNode)
        flag = True
